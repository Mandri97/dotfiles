...
slurp_select_area="slurp -d -b '#ebdbb244' -c '#1b1a1dDD' -B '#1b1a1dDD' -F 'Iosevka Nerd Font' -w '1'"
...
shot5 () {
	countdown '5'
        sleep 0.2 && grim -g "$(eval $slurp_select_area)" - | convert - -trim +repage - | swappy -f -
	notify_view "$1"
}

shot10 () {
	countdown '10'
	sleep 1 && grim -g "$(eval $slurp_select_area)" - | convert - -trim +repage - | swappy -f -
	notify_view "$1"
}
